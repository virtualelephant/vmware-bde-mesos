...
execute "Starting mesos-master" do
  command "nohup /usr/bin/mesos-init-wrapper master &"
end

#
# Install and setup Marathon & Chronos
# 

%w( marathon chronos ).each do |pkg|
  yum_package pkg do
    action :install
  end
end

execute "Starting Marathon" do
  command "nohup /usr/local/bin/marathon &"
end
